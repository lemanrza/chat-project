...
export const deleteMessage = async (messageId, userId) => {
    try {
        ...
        const deletedMessage = await MessageModel.findByIdAndUpdate(messageId, {
            $set: {
...
export const markMessageAsRead = async (messageId, userId) => {
    try {
        ...
        const alreadyRead = message.seenBy.some((seen) => seen.user.toString() === userId);
        if (alreadyRead) {
            return {
                success: true,
        ...
        const updatedMessage = await MessageModel.findByIdAndUpdate(messageId, {
            ...
            $set: { status: "read" },
        }, { new: true }).populate("seenBy.user", "username profile");
        return {
            success: true,
            data: updatedMessage,
...
export const addReaction = async (messageId, userId, emoji) => {
    try {
        ...
        if (existingReaction) {
            ...
            }, { new: true }).populate("reactions.user", "username profile");
            return {
                success: true,
                data: updatedMessage,
...
export const searchMessages = async (chatId, userId, query, page = 1, limit = 20) => {
    try {
        const chat = await ChatModel.findOne({
            _id: new Types.ObjectId(chatId),
            "members.user": new Types.ObjectId(userId),
