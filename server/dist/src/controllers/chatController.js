import { createChat, getUserChats, getChatById, updateChat, addMemberToChat, removeMemberFromChat, archiveChat, deleteChat, searchChats, } from "../services/chatService.js";
...
export const addChatMember = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const { chatId } = req.params;
...
export const archiveChatById = async (req, res, next) => {
    try {
        const userId = req.user.id;
        const { chatId } = req.params;
        if (!chatId) {
