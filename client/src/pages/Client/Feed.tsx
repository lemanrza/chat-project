        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredUsers.map((userData) => {
              ...
              // Handle connectionsRequests as either string[] or UserData[]
              const userConnectionRequests = Array.isArray(userData.connectionsRequests)
                ? userData.connectionsRequests.map(req => typeof req === 'string' ? req : (req as UserData).id)
                : [];
                
              const isRequestPending = user.id ? userConnectionRequests.includes(user.id) : false;
